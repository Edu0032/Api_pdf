...
from app.bases.sicro.composicoes_parser import parse_composicoes_sicro
...
_GROUP_BLACKLIST = ("SICRO", "PRÓPRIO", "PROPRIO", "UND", "QUANT", "CUSTO", "BDI", "%")
...
def parse_sicro(
    ...
    if not (c_ini and c_fim and c_ini >= 1 and c_fim >= c_ini):
        ...
        comp, comp_avisos, comp_erros, itens_faltando, itens_extras = parse_composicoes_sicro(
            pdf_bytes=pdf_bytes,
            start_1based=c_ini,
            end_1based=c_fim,
            config=config,
...
def _make_item_start_regex(config: dict) -> re.Pattern:
    syn = config.get("synthetic", {}) if isinstance(config, dict) else {}
    sources = syn.get("sources") or ["SICRO", "PRÓPRIO", "PROPRIO"]
    code_pattern = syn.get("code_pattern") or r"[0-9A-Z_\-\/\.]+"

    src_alt = "|".join(re.escape(s) for s in sources)
    return re.compile(
        rf"^(?P<item>\d+(?:\.\d+)*)\s+(?P<codigo>{code_pattern})\s+(?P<fonte>{src_alt})\s+(?P<rest>.+)$",
        re.IGNORECASE,
    )
...
def _parse_orcamento_sintetico(
    ...
    _RE_ITEM_START = _make_item_start_regex(config)
    ...
    for ln in raw_lines:
        if not started:
            ...
            if "TOTAL" in ln.upper() and ("BDI" in ln.upper()):
                break
            lines.append(ln)
    ...
    def try_finalize_item(buffer_lines: List[str], lookahead_lines: List[str]) -> tuple[Optional[dict], int]:
        ...
        for extra_used in range(0, max_extra + 1):
            text = " ".join(cur)
            parsed = _try_parse_item_row(text, _RE_ITEM_START, strip_inline_from, dynamic_markers)
            if parsed:
        ...
        if mg:
            ...
            if not _is_probable_group_heading(desc):
                if last_item_node and is_safe_continuation(
                    ...
                    push_node({"tipo": tipo, "item": item, "descricao": desc, "custo_total": (total or "").strip(), "filhos": []})
                i += 1
                continue
...
def _try_parse_item_row(
    text: str,
    _RE_ITEM_START: re.Pattern,
...
def _collect_item_refs(itens_raiz) -> List[Dict[str, str]]:
    """
    Aceita árvore como dict OU OrcamentoItem (Pydantic).
    Retorna:
      [{"item":"1.1.1","ref_id":"CODIGO|FONTE"}, ...]
    """
    ...
    def children(node):
        ...
        return getattr(node, "filhos", None) or []

    def walk(nodes):
        for n in nodes or []:
            tipo = str(getv(n, "tipo", "")).strip().lower()
            if tipo == "item":
