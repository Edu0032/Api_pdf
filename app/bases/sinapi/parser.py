...
def _expand_orcamento_pages_if_needed(pdf_bytes: bytes, start_1based: int, end_1based: int, pages_text: List[str]) -> tuple[List[str], int, bool]:
    ...
    while current_end < len(reader.pages) and current_end < end_1based + 2:
        next_text = reader.pages[current_end].extract_text() or ""
...
def parse_sinapi(
    ...
    if o_ini and o_fim and o_ini >= 1 and o_fim >= o_ini:
        ...
        pages_text, o_fim_processado, expanded_orc = _expand_orcamento_pages_if_needed(pdf_bytes, o_ini, o_fim, pages_text)
        orc, a, e, d = _parse_orcamento_sintetico(pages_text, config=config, context=context)
        avisos.extend(a)
        erros.extend(e)
        divergencias.extend(d)
        if expanded_orc:
            avisos.append(f"[orcamento] intervalo expandido automaticamente até a página {o_fim_processado} para capturar o fim do anexo sintético.")
    else:
...
def _collect_item_refs(itens_raiz) -> List[Dict[str, str]]:
    ...
    Ignora:
      ...
      - códigos que parecem INSUMO (ex.: 0000xxxx) — esses devem virar aviso,
        não referência de composição principal.
    """
        ...
        for n in nodes or []:
            ...
            if tipo == "item":
                ...
                if item and codigo and banco:
                    if codigo_upper != "COMPOSICAO" and not _is_probably_insumo_codigo(codigo, banco):
                        refs.append(
                            {
                                "item": item,
                                "codigo": codigo_upper,
                                "fonte": banco,
                                "ref_id": f"{codigo_upper}|{banco}",
                            }
                        )
